...
const TextType = ({
  ...
  // Check if typing is complete (reached the end of all text)
  ...
  const renderTextWithLineBreaks = (text: string) => {
    return text.split("\n").map((line, index, array) => (
      <span key={index}>
        <span
          className={index === 0 ? "text-white" : "text-gradient-pink"}
          style={{
            color: index === 0 ? "#ffffff" : undefined,
            background: index === 1 ? "var(--gradient-pink)" : undefined,
            WebkitBackgroundClip: index === 1 ? "text" : undefined,
            WebkitTextFillColor: index === 1 ? "transparent" : undefined,
            backgroundClip: index === 1 ? "text" : undefined,
          }}
        >
          {line}
        </span>
